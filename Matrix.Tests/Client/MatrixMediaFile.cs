...
namespace Matrix.Tests.Client
{
    ...
    public class MatrixMediaFileTests
    {
        ...
        public void TestCreateMediaFile()
        {
            ...
            MatrixMediaFile media = new MatrixMediaFile(mock.Object, MXC_URL, CONTENT_TYPE);
            Assert.That(media.GetUrl(), Is.EqualTo("https://localhost/_matrix/media/r0/download/half-shot.uk/oSnvUaEqIQcsVfAuulWeeBVB"));
            Assert.That(media.GetThumbnailUrl(256,256), Is.EqualTo("https://localhost/_matrix/media/r0/thumbnail/half-shot.uk/oSnvUaEqIQcsVfAuulWeeBVB?width=256&height=256&method=crop"));
        }
    }
}